...
class Entity extends DBManager{
  ...
  private function filter_gets($ignored = array()){
    ...
    foreach ($_GET as $key => $value) {
      if (!in_array($key, $ignored)){
        if (is_array($value)){
          $key = $this->convert_array_key($key, $value);
          $value = $this->convert_array_value($value);
        }
        $query .= $key.'='.$value;
      }
      if ($count > 0){
  ...
  private function convert_array_value($thekey){
    $output = "";
  ...
  private function convert_array_key($thekey, $thevalue){
    $output = "";
